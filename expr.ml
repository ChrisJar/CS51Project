...
let rec replace exp newv oldv =
  match exp with
    ...
    | Let(v,e1,e2) -> if v = oldv then Binop(newv,e1,(replace e2 newv oldv))
                      else Binop(v,(replace e1 newv oldv),(replace e2 newv oldv))
    | Letrec(v,e1,e2) -> if v = oldv then Binop(newv,(replace e1 newv oldv),(replace e2 newv oldv))
                         else Binop(v,(replace e1 newv oldv),(replace e2 newv oldv))
    | Raise -> Raise
...
let subst (var_name: varid) (repl: expr) (exp: expr) : expr =
  let freerepl = free_vars repl in
    let rec sub ex =
      ...
      | Fun(v,e) -> if v = var_name then ex else
                      (if SS.mem v freerepl then sub (replace ex (gensym ()) v)
                      else Fun(v,(sub e)))
      | Let(v,e1,e2) -> if v = var_name then Let(v,(sub e1),e2) else
                          (if SS.mem v freerepl then sub (replace ex (gensym ()) v)
                          else Let(v,(sub e1),(sub e2)))
      | Letrec(v,e1,e2) -> if v = var_name then Letrec(v,e1,e2) else
                             (if SS.mem v freerepl then sub (replace ex (gensym ()) v)
